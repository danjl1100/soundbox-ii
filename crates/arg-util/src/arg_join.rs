...
pub use self::display_as_debug::DisplayAsDebug;
pub use self::str_debug_split::StrDebugSplit;
pub use self::string_debug_split::StringDebugSplit;
...
mod str_debug_split {
    use crate::arg_split::{BACKSLASH, DOUBLE_QUOTE, SINGLE_QUOTE};
...
mod string_debug_split {
    use super::StrDebugSplit;
...
mod display_as_debug {
    /// Wrapper to display the inner value as a debug
    #[derive(Clone, PartialEq, serde::Serialize, serde::Deserialize, arbitrary::Arbitrary)]
    #[serde(transparent)]
    pub struct DisplayAsDebug<T>(pub T);
    // Display uses inner-Debug
    impl<T> std::fmt::Display for DisplayAsDebug<T>
    where
        T: std::fmt::Debug,
    {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            let Self(inner) = self;
            <T as std::fmt::Debug>::fmt(inner, f)
        }
    }
    // Transparent trait impls
    impl<T> std::str::FromStr for DisplayAsDebug<T>
    where
        T: std::str::FromStr,
    {
        type Err = T::Err;
    ...
    impl<T> std::fmt::Debug for DisplayAsDebug<T>
    where
        T: std::fmt::Debug,
    {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
